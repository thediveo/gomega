		...
		BeforeEach(func() {
			...
			When("passed an iter.Seq2", func() {
				It("should do the right thing", func() {
					...
					Expect(universalMapIter2).Should(HaveKeyWithValue("bar", 666))
			...
			When("the passed in key or value is actually a matcher", func() {
				It("should pass each element through the matcher", func() {
					...
					Expect(universalMapIter2).Should(HaveKeyWithValue(ContainSubstring("foo"), 2))
				})

				It("should fail if the matcher ever fails", func() {
					success, err := (&HaveKeyWithValueMatcher{Key: 2, Value: 2}).Match(universalMapIter2)
					Expect(success).Should(BeFalse())
					Expect(err).Should(HaveOccurred())
